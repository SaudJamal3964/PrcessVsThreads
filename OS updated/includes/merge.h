...
int fd1[PROCESS_MAX][2];       // fileDiscripters that will be used for implementing pipes
...
void mergeSortThread() {
    ...
    printf("--------------------------------------------------------\n");
    mtt=(float)start / CLOCKS_PER_SEC;
}
...
void msProcess() {
    ...
    write(fd1[factor][1], &tempArray, mSizeArray[factor] * sizeof(int));
}  // function that performs merge-sort on 4 different partitions
...
void mergeSortProcess() {
    ...
    if (process[0] == 0) {
        ...
        /* reading from pipes */
        read(fd1[0][0], &process1Array, mSizeArray[0] * sizeof(int));
        read(fd1[1][0], &process2Array, mSizeArray[1] * sizeof(int));
        read(fd1[2][0], &process3Array, mSizeArray[2] * sizeof(int));
        read(fd1[3][0], &process4Array, mSizeArray[3] * sizeof(int));
        ...
        if (process[0] == 0) {
            ...
            write(fd1[0][1], &firstHalf, mid * sizeof(int));
            exit(0);
            ...
            if (process[1] == 0) {
                ...
                write(fd1[1][1], &secondHalf, (numberOfElements - mid) * sizeof(int));
                exit(0);
            }
            ...
            // reading from pipes
            read(fd1[0][0], &firstHalf, mid * sizeof(int));
            read(fd1[1][0], &secondHalf, (numberOfElements - mid) * sizeof(int));
        ...
        printf("\nTime taken for sorting using process: %f seconds\n", (float)start / CLOCKS_PER_SEC);
            	printf("--------------------------------------------------------\n");
	mpt=(float)start / CLOCKS_PER_SEC;
    }
}
#endif
